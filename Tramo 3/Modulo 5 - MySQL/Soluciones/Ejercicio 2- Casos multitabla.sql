...
group by p.codigo having count(p.codigo_fabricante);

SELECT f.nombre FROM fabricante f INNER JOIN producto p ON f.codigo = p.codigo_fabricante
GROUP BY f.nombre HAVING COUNT(f.nombre = 'Lenovo') = COUNT(p.codigo_fabricante);

SELECT f.nombre FROM fabricante f JOIN producto p ON f.codigo = p.codigo_fabricante GROUP BY f.codigo, f.nombre
HAVING COUNT(p.codigo) = (SELECT COUNT(p2.codigo) FROM fabricante f2 JOIN producto p2 ON f2.codigo = p2.codigo_fabricante
